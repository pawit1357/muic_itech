...
$(function(){
	var availableTags = [
		             		<?php 
		             		$userLogins = UserLogin::model()->findAll();
							if($userLogins != null) {
...
function changeMode() {
	if($('#mode').val() == 'daily') {
		...
		window.location = "<?php echo Yii::app()->createUrl('Report/UserStatisticReport')?>" + "/username/" + $('#username').val() + "/mode/yearly";
	}
}
function filter() {
	if($('#mode').val() == 'daily') {
		if($('#month_filter').val() != '' && $('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/UserStatisticReport')?>" + "/username/" + $('#username').val() + "/mode/daily/month_filter/" + $('#month_filter').val() + "/year_filter/" + $('#year_filter').val();
		}
	} else if ($('#mode').val() == 'monthly') {
		if($('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/UserStatisticReport')?>" + "/username/" + $('#username').val() + "/mode/monthly/year_filter/" + $('#year_filter').val();
		}
	}
}
function loadDaily() {
...
?>
<span class="module-head">Request Booking Statistic Report</span>
<div>
	<form>
		User : <input class="input-short1" type="text"
			value="<?php echo $_GET['username']?>" id="username" name="username">
		| 
...
<div>
	<table class="report">
		...
		if($username != '') {
			...
			$sql = "SELECT * FROM (";
		$sql = $sql."SELECT count(request_booking.id) as count_id, 'Request Booking' as module_name, ";
		if($mode == 'daily') {
			$sql = $sql." DATE_FORMAT(create_date,'%d') as c_date ";
		} else if ($mode == 'monthly') {
			$sql = $sql." DATE_FORMAT(create_date,'%b') as c_date ";
		} else if ($mode == 'yearly') {
			$sql = $sql." DATE_FORMAT(create_date,'%Y') as c_date ";
		}
		$sql = $sql." FROM request_booking INNER JOIN user_login ON request_booking.user_login_id = user_login.id WHERE user_login.username = '".$username."'";
		if($fromDate != '' && $thruDate != '' && strtotime($fromDate) > 0 && strtotime($thruDate) > 0) {
			$sql = $sql." AND create_date between '".$fromDate."' AND '".$thruDate."'";
		}
		$sql = $sql." GROUP BY c_date ";
		$sql = $sql." UNION SELECT count(request_borrow.id) as count_id, 'Request Borrow' as module_name, ";
		if($mode == 'daily') {
			$sql = $sql." DATE_FORMAT(create_date,'%d') as c_date ";
		} else if ($mode == 'monthly') {
			$sql = $sql." DATE_FORMAT(create_date,'%b') as c_date ";
		...
		$sql = $sql." FROM request_borrow INNER JOIN user_login ON request_borrow.user_login_id = user_login.id WHERE user_login.username = '".$username."'";
		if($fromDate != '' && $thruDate != '' && strtotime($fromDate) > 0 && strtotime($thruDate) > 0) {
			$sql = $sql." AND create_date between '".$fromDate."' AND '".$thruDate."'";
		}
		$sql = $sql." GROUP BY c_date ";
		$sql = $sql." UNION SELECT count(request_service.id) as count_id, 'Request Service' as module_name, ";
		if($mode == 'daily') {
			$sql = $sql." DATE_FORMAT(create_date,'%d') as c_date ";
		} else if ($mode == 'monthly') {
			$sql = $sql." DATE_FORMAT(create_date,'%b') as c_date ";
		...
		$sql = $sql." FROM request_service INNER JOIN user_login ON request_service.user_login_id = user_login.id WHERE user_login.username = '".$username."'";
		if($fromDate != '' && $thruDate != '' && strtotime($fromDate) > 0 && strtotime($thruDate) > 0) {
			$sql = $sql." AND create_date between '".$fromDate."' AND '".$thruDate."'";
		}
		$sql = $sql." GROUP BY c_date ";
		$sql = $sql.") AS a ORDER BY a.c_date";
...
$data_array2 = array();
	while($item = mysql_fetch_assoc($result)){
		...
		echo '<tr>';
		$data_array[$item['module_name']] = $data_array[$item['module_name']] + $item['count_id'];
		$data_array2[$item['module_name']][$item['c_date']] = $item['count_id'];
	}
	if($count == 0) {
...
foreach($data_array2 as $key => $value){
	...
	$data_chart2 = $data_chart2."]},";
			...
			echo '<tr><td colspan="3" align="center"><i>- Chose User -</i></td></tr>';
		}
		?>
	</table>
