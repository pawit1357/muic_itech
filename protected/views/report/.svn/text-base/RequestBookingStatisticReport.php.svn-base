...
function changeMode() {
	if($('#mode').val() == 'daily') {
		...
		window.location = "<?php echo Yii::app()->createUrl('Report/RequestBookingStatisticReport')?>" + "/mode/yearly";
	}
}
function filter() {
	if($('#mode').val() == 'daily') {
		if($('#month_filter').val() != '' && $('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBookingStatisticReport')?>" + "/mode/daily/month_filter/" + $('#month_filter').val() + "/year_filter/" + $('#year_filter').val();
		}
	} else if ($('#mode').val() == 'monthly') {
		if($('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBookingStatisticReport')?>" + "/mode/monthly/year_filter/" + $('#year_filter').val();
		}
	}
}
function loadDaily() {
...
<div>
	<table class="report">
		...
		if(($mode == 'daily' || $mode == 'monthly' || $mode == 'yearly') && ($fromDate == '' || strtotime($fromDate) > 0) && ($thruDate == '' || strtotime($thruDate) > 0)) {
			...
			$sql = "SELECT count(id) as count_id, request_booking_type_id, ";
			if($mode == 'daily') {
		...
		$count = $count +1;
		if($item['request_booking_type_id'] == '1') {
			$requestType = 'Daily';
		} else if ($item['request_booking_type_id'] == '2') {
			$requestType = 'Semester';
		} else if ($item['request_booking_type_id'] == '3') {
			$requestType = 'Activity / Meeting';
		}
		echo '<tr>';
		echo '<td>'.$item['c_date'].'</td>';
		echo '<td align="center">'.$requestType.'</td>';
		echo '<td align="right">'.$item['count_id'].'</td>';
		echo '<tr>';
		$data_array[$requestType] = $data_array[$requestType] + $item['count_id'];
		$data_array2[$requestType][$item['c_date']] = $item['count_id'];
	}
	if($count == 0) {
