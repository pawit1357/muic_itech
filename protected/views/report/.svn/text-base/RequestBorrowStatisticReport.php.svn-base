...
function changeMode() {
	if($('#mode').val() == 'daily') {
		...
		window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowStatisticReport')?>" + "/mode/yearly";
	}
}
function filter() {
	if($('#mode').val() == 'daily') {
		if($('#month_filter').val() != '' && $('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowStatisticReport')?>" + "/mode/daily/month_filter/" + $('#month_filter').val() + "/year_filter/" + $('#year_filter').val();
		}
	} else if ($('#mode').val() == 'monthly') {
		if($('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowStatisticReport')?>" + "/mode/monthly/year_filter/" + $('#year_filter').val();
		}
	}
}
function loadDaily() {
...
?>
<span class="module-head">Request Borrow Statistic Report</span>
<div>
	<form>
...
$mode = $_GET['mode'];
	...
	while($item = mysql_fetch_assoc($result)){
		...
		$data_array2['Request Borrow'][$item['c_date']] = $item['count_id'];
	}
	if($count == 0) {
		echo '<tr><td colspan="2" align="center"><i>- Item Not Found -</i></td></tr>';
	} else {
...
$data_chart = $data_chart."['".$key."', ".$value."], ";
	...
	foreach($data_array2 as $key => $value){
		...
		$data_chart2 = $data_chart2."]},";
			...
			echo '<tr><td colspan="2" align="center"><i>- Invalid Parameter -</i></td></tr>';
		}
		?>
	</table>
