...
function changeMode() {
	if($('#mode').val() == 'daily') {
		...
		window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowUsingStatisticReport')?>" + "/mode/yearly";
	}
}
function filter() {
	if($('#mode').val() == 'daily') {
		if($('#month_filter').val() != '' && $('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowUsingStatisticReport')?>" + "/mode/daily/month_filter/" + $('#month_filter').val() + "/year_filter/" + $('#year_filter').val();
		}
	} else if ($('#mode').val() == 'monthly') {
		if($('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestBorrowUsingStatisticReport')?>" + "/mode/monthly/year_filter/" + $('#year_filter').val();
		}
	}
}
function loadDaily() {
...
?>
<span class="module-head">Request Borrow Using Equipment Statistic Report</span>
<div>
	<form>
...
$mode = $_GET['mode'];
		...
		if(($mode == 'daily' || $mode == 'monthly' || $mode == 'yearly') && ($fromDate == '' || strtotime($fromDate) > 0) && ($thruDate == '' || strtotime($thruDate) > 0)) {
			...
			$sql = "SELECT sum(request_borrow_equipment_type.quantity) as count_id, equipment_type.name as name, ";
			if($mode == 'daily') {
		$sql = $sql." DATE_FORMAT(request_borrow.create_date,'%d') as c_date ";
	} else if ($mode == 'monthly') {
		$sql = $sql." DATE_FORMAT(request_borrow.create_date,'%b') as c_date ";
	} else if ($mode == 'yearly') {
		$sql = $sql." DATE_FORMAT(request_borrow.create_date,'%Y') as c_date ";
	}
	$sql = $sql." FROM request_borrow INNER JOIN request_borrow_equipment_type ON request_borrow.id = request_borrow_equipment_type.request_borrow_id INNER JOIN equipment_type ON request_borrow_equipment_type.equipment_type_id = equipment_type.id";
