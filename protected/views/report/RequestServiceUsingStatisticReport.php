...
function changeMode() {
	if($('#mode').val() == 'daily') {
		...
		window.location = "<?php echo Yii::app()->createUrl('Report/RequestServiceUsingStatisticReport')?>" + "/mode/yearly";
	}
}
function filter() {
	if($('#mode').val() == 'daily') {
		if($('#month_filter').val() != '' && $('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestServiceUsingStatisticReport')?>" + "/mode/daily/month_filter/" + $('#month_filter').val() + "/year_filter/" + $('#year_filter').val();
		}
	} else if ($('#mode').val() == 'monthly') {
		if($('#year_filter').val() != '') {
			window.location = "<?php echo Yii::app()->createUrl('Report/RequestServiceUsingStatisticReport')?>" + "/mode/monthly/year_filter/" + $('#year_filter').val();
		}
	}
}
function loadDaily() {
...
?>
<span class="module-head">Request Service Statistic Report</span>
<div>
	<form>
...
<div>
	<table class="report">
		...
		if(($mode == 'daily' || $mode == 'monthly' || $mode == 'yearly') && ($fromDate == '' || strtotime($fromDate) > 0) && ($thruDate == '' || strtotime($thruDate) > 0)) {
			...
			$sql = "SELECT sum(request_service.quantity) as count_id, request_service_type.name as name, ";
			if($mode == 'daily') {
		$sql = $sql." DATE_FORMAT(request_service.create_date,'%d') as c_date ";
	} else if ($mode == 'monthly') {
		$sql = $sql." DATE_FORMAT(request_service.create_date,'%b') as c_date ";
	} else if ($mode == 'yearly') {
		$sql = $sql." DATE_FORMAT(request_service.create_date,'%Y') as c_date ";
	}
	$sql = $sql." FROM request_service INNER JOIN request_service_detail ON request_service.id = request_service_detail.request_service_id INNER JOIN request_service_type_detail ON request_service_detail.request_service_type_detail_id = request_service_type_detail.id INNER JOIN request_service_type ON request_service_type.id = request_service_type_detail.request_service_type_id";
	...
	while($item = mysql_fetch_assoc($result)){
		$count = $count +1;
		echo '<tr>';
		echo '<td align="center">'.$item['c_date'].'</td>';
