<div class="module-head">Present Type</div>
<div>
	<?php 
	if(UserLoginUtil::hasPermission(array("FULL_ADMIN", "CREATE_POSITION"))){
		echo CHtml::link('Create New',array('PresentType/Create'),array('class'=>'add'));
	}
	?>
		...
		?>
			...
			<td class="center"><?php echo GridUtil::getDataIndex($dataProvider, $counter++)?></td>
				...
				<td class="center"><a title="View" class="ico-s-view"
					href="<?php echo Yii::app()->CreateUrl('PresentType/view/id/'.$data->id)?>"></a>
					<a title="Edit" class="ico-s-edit"
					href="<?php echo Yii::app()->CreateUrl('PresentType/update/id/'.$data->id)?>"></a>
					<a title="Delete"
					onclick="return confirm('Are you sure to delete?')"
					class="ico-s-delete"
					href="<?php echo Yii::app()->CreateUrl('PresentType/delete/id/'.$data->id)?>"></a>
				</td>
			</tr>
			<?php 
		}
